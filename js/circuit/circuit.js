...
function createCanvas() {
  ...
  img.scale.set((window.innerWidth / window.innerHeight) * 0.6);
...
async function load() {
  ...
  if (!parcoursDisponibles.includes(parcours))
    return window.location.redirect("/"); // Regarde si on peut récuperer le parcours, sinon redirige
